	...
	render() {
		...
		return (
			<div style={center} title={name} className="marker">
				<Link to={{pathname: name.replace(/\s/g, '_'), state: {type: type, name: name, photo: photo, source: source} }} tabIndex="-1"><img src={require(`../icons/${type === "City" ? "Cities/" + name : type}.png`)} alt={name}/></Link>
			</div>
		)
	}
}
